...
| Path outside the active ZIP selection | Card renders muted with `OUTSIDE SELECTION` (never hidden). An image tile keeps its pixels at full brightness; only its badge, counter, and name mute and its slot border goes dashed. |
