...
| Discard All (CHANGES section header, discard glyph next to stage-all) | Confirm modal (destructive) stays compact — counts only (N restored from the index, M deleted), never per-path lines — then sends one batched discard whose targets carry the same per-file stamps the row action sends; disabled while an action is pending, status is truncated, or CHANGES is empty. |
