...
| Push or pull fails without proof that nothing changed (a remote rejection, a timeout) | The column reconciles (`PUSH RESULT UNKNOWN — REFRESHING`) and, when the agent relayed Git's text, shows it at once as a `PUSH DID NOT FINISH` notice (for example `! [remote rejected] master -> master (Working directory has unstaged changes)`), so the reason is never lost behind the reconciliation. |
| Discard All (CHANGES section header, discard glyph next to stage-all) | Confirm modal (destructive) states the count and lists up to 15 target paths with their effect (`…and N more` beyond that), then sends one batched discard whose targets carry the same per-file stamps the row action sends; disabled while an action is pending, status is truncated, or CHANGES is empty. |
