...
- Stream panel (2026-09-06, 0.1.23): the left activity panel gained a STREAM mode, now the default, that prints what agents edit as they edit it — hunk cards in the workspace diff grammar, all-added new files, struck-through deletions, image thumbnails dropping in, burst cards for checkouts, a twenty-card ring, click-to-expand and double-click-to-open — fed by one additive `fileDelta` event from a bounded delta pipeline in the repo watcher (settle, budgeted read, `similar` diff against the previous sighting or the index blob). Persisted as `uiState.filesMode`. Contract `docs/design/stream_panel_design.md`; witness `docs/commands/verify_stream.md`. A sixth external review on 2026-09-06 (`docs/reports/stream_adversarial_review_20260906.md`) found no P0, six P1 contract failures and one P2, all accepted and all closed in the same round: the index baseline is read before the worktree and carried across re-settles, image pixels are bound to their revision by `mtimeMs` with the size gate enforced at the byte owner, the read budget refills causally so one flood costs one budget, an overload clears the baseline cache rather than diffing against text it stopped tracking, both agents' per-connection event queues are bounded with drop counters while `fileDeltaCounters` joins the per-repo sequence so every drop is a visible gap, and a read permit now lives inside the blocking job it bounds. **0.1.23 is a candidate build: it is not accepted until JL has run the installed-app witness.**
