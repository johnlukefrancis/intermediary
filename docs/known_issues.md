...
- 2026-02-11: Screenshot/image files with common extensions (`.png`, `.jpg`, `.jpeg`, `.webp`, etc.) are now classified as images so they appear in File Intelligence instead of being filtered as `other`.
...
- 2026-02-06: Narrow code classifier coverage could miss language families (for example `*.cpp`) in File Intelligence. Fixed by generated broad-language extension coverage, default-only codeGlobs migration, and a separate classification-excludes model in Options.
