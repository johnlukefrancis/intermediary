...
- 2026-09-07: The TypeScript legacy global-excludes normalizer re-merged every recommended exclude into any stored config whose version was below the current one, so each config-version bump would have re-added entries the user had removed (dormant in the product only because the Rust loader bumps the version first). Fixed by gating it to pre-v7 preset-era configs; v7+ lists are the user's authority. Recommended additions now flow only through the version-keyed additions migration; `.blend`/`.blend1` are seeded into every config once at v28 after the baseline-gated v27 step left JL's trimmed config without them.
