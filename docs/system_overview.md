...
- **Key features:**
  ...
  - Two-column layout per repo: File Intelligence and Zip Bundles
  ...
  - Shared workspace replaces the File Intelligence panel for supported UTF-8 file scratch buffers or supported image previews; scratch edits never write back to repo files
  - File Intelligence exposes Intelligence/Latest/Active sort modes plus All/Documents/Code/Images icon filters; rows render persisted activity bars, trend, last active time, update count, and a 24-hour pulse strip
  ...
  - Zip Bundles column includes a lazy file explorer: root files are visible, expanded directories fetch direct child files/subdirectories on demand, file icon clicks toggle bundle inclusion, and file-name context menus reuse the same OS file actions as File Intelligence rows
...
- **Key features:**
  ...
  - Recent changes index with 250ms debouncing, persisted history under `staging/state/recent_files/<repoId>.json`, and per-file activity metadata for File Intelligence ranking
...
**Agent → UI events:**
- `fileChanged { repoId, path, kind, changeType, mtime, activity?, staged? }`, where `activity.history` carries 24-hour bucket data for File Intelligence telemetry
...
- Classification excludes are applied at watcher time to suppress noisy/generated files in File Intelligence.
...
1. **File Change → UI Update:** Repo file changes → backend watcher (Windows local or WSL) → host agent event bus → UI updates the File Intelligence table from the unified recent list; topology-changing directory events also refresh bundle explorer root metadata
