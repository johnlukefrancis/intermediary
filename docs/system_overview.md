...
- **Key features:**
  ...
  - Zip Bundles column includes a lazy file explorer: root files are visible, expanded directories fetch
    direct child files/subdirectories on demand, file icon clicks toggle bundle inclusion, and file-name
    context menus reuse the same OS file actions as Auto Files rows. Changed files and directories carry
    Git-status decorations (tinted name + change-letter badge on files, tinted name + changed-path count on
    directories) derived from the same source-control status that feeds the SOURCE count; expanded
    directories re-list on `sourceControlChanged` so newly created files appear with their badge
  - Drag-in import: OS files or folders dropped onto the Zip Bundles explorer tree are copied into the
    targeted directory of the active repo by the agent that owns the root (`importFiles`, held under the
    Source Control mutation lock); hovering a collapsed directory for about 700 ms during the drag expands
    it, existing names are refused and confirmed through a Replace modal, nothing is staged in Git, and
    the ordinary watcher events surface the new files in the tree, Auto Files, and SOURCE. The main window
    enables Tauri's native drag-drop so the webview receives the OS paths; it never reads or writes them
  - Tree selection and worktree actions: ZIPS rows select (click / Ctrl / Shift, a plain folder click also
    toggles expansion; the checkbox alone controls bundle inclusion), with keyboard navigation, cut/copy/paste,
    delete (to the repo's discard quarantine, recoverable until the next agent start), drag-move within the
    tree (a pointer drag with a ghost, sharing the OS drop's hit-test and hover-to-expand), and inline rename —
    all through one `worktreeAction` command executed by the root-owning agent under the Source Control
    mutation lock; nothing is staged in Git and `.git` is never a source or destination
...
- **Key features:**
  ...
  - On `shutdown` (app stop/restart/exit, SIGTERM, ctrl-c), forwards `shutdown` to the WSL backend first, then drains its own in-flight source-control mutations before responding and exiting; new mutations requested while draining return `AGENT_DRAINING` while reads are still served; `drained: false` never triggers an exit — the agent keeps waiting up to a 450 s emergency bound (then terminates its owned process tree and logs `unknown` with the residue), and `WSL_BACKEND_UNAVAILABLE` counts as drained only when no forwarded mutation request id is still outstanding to that backend
...
- `listRepoDirectory { repoId, path } → listRepoDirectoryResult`; `path` is repo-relative and `""` lists the repo root, while non-root paths return direct child directories/files as repo-relative paths; `.git` is never returned as a directory and a path with a `.git` component is `INVALID_PATH`
- `importFiles { repoId, directory, sources, onConflict: "refuse" | { replace: string[] } } → importFilesResult { repoId, directory, imported: [{ path, bytes }] }`; `directory` is repo-relative (`""` = root) and `sources` are absolute OS paths as the drop delivered them (the WSL agent translates `C:\…` and same-distro `\\wsl$\…` / `\\wsl.localhost\…` forms; any other UNC is `IMPORT_UNSUPPORTED_SOURCE`). Validation and a conflict pre-pass run before any write: `ENTRY_CONFLICT` (`details.conflicts`) for an existing destination file under `refuse` (a dropped folder merges into an existing folder, so conflicts are per resulting file) or for two sources resolving to one destination, `IMPORT_SOURCE_NOT_FOUND`, `IMPORT_UNSUPPORTED_SOURCE` (relative path, symlink, `.git`, or a source at or above its own destination), `ENTRY_KIND_MISMATCH` (a file would land on an existing folder or a folder on an existing file, refused under either policy), `IMPORT_TOO_LARGE` (10,000 entries), all `effect: notApplied`; a mid-copy IO failure is `INTERNAL_ERROR` with `effect: unknown` and `details.imported`. A replace carries the reviewed conflict list; the agent recomputes the live collisions and refuses again (fresh list) if any is unauthorized, then only authorized destinations are written with temp + rename while every other destination streams into a `create_new` file (no temp file, so a racing writer surfaces as a conflict). The destination, like every planned entry, is refused with `INVALID_PATH` if it carries a `.git` component; a dropped folder containing a `.git` directory is refused whole. The import holds the per-git-dir mutation lock (a non-Git root refuses with `GIT_NOT_REPOSITORY`), counts in the drain ledger, is non-cancellable, and is bounded host→WSL at 300 s / UI 330 s — a copy budget rather than a proof; on expiry the effect is `unknown` and the watcher reconciles
- `worktreeAction { repoId, action } → worktreeActionResult { repoId, kind, entries }`; `action.kind` is `delete { paths }` (each entry, file or whole folder, is claimed by rename into `<git dir>/intermediary-discard/<opId>-<i>/claimed`, marked `verified` with plan `delete`, and left `retained` until the next agent start's sweep — no stamp check, the user named it; the registry remembers every quarantine op this process created, so this process's sweep never removes it), `move { paths, directory, onConflict }` (filesystem renames — `rename_no_replace` unless the destination is in the authorized replace list, so a racing destination is a conflict and drvfs through the WSL agent is `SOURCE_CONTROL_UNSUPPORTED_LAYOUT`; file over file refused under `refuse` or replaced when authorized, folder over folder always `ENTRY_CONFLICT`, cross-kind `ENTRY_KIND_MISMATCH`, a folder into itself `INVALID_PATH`, an entry already in the destination is a no-op), `copy { paths, directory, onConflict }` (the import writer over in-repo sources, so folders merge and a same-folder copy is refused), and `rename { path, newName }` (`ENTRY_INVALID_NAME` for empty/`/`/`\`/NUL/`.`/`..`/`.git`; an existing destination is `ENTRY_CONFLICT`, never replaced — the rename itself is non-replacing; a case-only rename is recognised by filesystem identity and keeps the plain rename). `entries` lists the resulting repo-relative paths. Every refusal (`INVALID_PATH` for any `.git` component or root escape, `ENTRY_NOT_FOUND`, the codes above, `SOURCE_CONTROL_UNSUPPORTED_LAYOUT` when a rename would cross volumes) is proven before the first write with `effect: notApplied`; a failure after the first entry landed is `effect: unknown` with `details.applied`. Held under the per-git-dir mutation lock, counted in the drain ledger, non-cancellable; delete/move/rename are bounded host→WSL at 60 s / UI 90 s, copy at the import tier (300 s / 330 s)
...
- `sourceControlStatus { repoId } → sourceControlStatusResult { repoId, status }`; whole-repository porcelain-v2 status projected onto the configured root (staged paths, unmerged paths above the root, and non-UTF-8 paths are counted in `status.omitted`, `status.committable` is true only when nothing is unmerged and the index differs from HEAD or a merge is in progress, output over 8 MiB sets `status.truncated`); status also carries `status.indexTreeSha` (the tree `write-tree` would produce, read before and after the porcelain read with up to 3 retries on a mismatch — still torn → `""`), `status.snapshotId` (hex SHA-256 over the branch or `detached`, `headSha` or `unborn`, `indexTreeSha`, and the contents of `MERGE_HEAD`/`CHERRY_PICK_HEAD`/`REVERT_HEAD` — the single identity a commit is bound to, `""` when no stable snapshot was captured), `status.headSha` (`null` on an unborn branch), `status.mutationInProgress` (the physical mutation lock's current state), and per-entry `worktreeStamp?: { bytes, mtimeMs, mtimeNanos }` plus `worktreeMissing?: true` for worktree/conflict entries absent on disk
...
- `sourceControlImageDiff { repoId, path, originalPath?, area } → sourceControlImageDiffResult { repoId, path, area, before, after }`; `before`/`after` are each an `ImageDiffSide | null` with `source: "head" | "index" | "worktree" | "ours" | "theirs"` naming which snapshot the side came from, bounded at 12 MiB per side raw with a `truncated` flag when a side hits the bound, and `null` when a side has no blob at that snapshot (added/untracked, deleted, or a stage absent from a conflict) rather than an error Image diffs share the source-control read timeout class, and the host->WSL hop raises its WebSocket frame bound to 64 MiB so a two-sided response is not dropped in transit.
- `sourceControlAction { repoId, action } → sourceControlActionResult { repoId, kind, status, commitSha?, hookChangedPaths?, hookAddedPaths? }`; `action.kind` is `stage` / `unstage` (scope `all` or `paths`, empty `paths` rejected as `INVALID_PATH`), `discard { targets: [{ path, expectedStamp?, expectedMissing? }] }` (a stamped target is atomically claimed into the operation's quarantine directory and verified there before it is touched, then destroyed only after a `verified` marker is written and kept as `retained` bytes until the next agent start; refused with `SOURCE_CONTROL_STATE_CHANGED` when a stamp mismatches, when a path marked `expectedMissing` now exists, or when a present path carries no assertion the review could stamp, and with `SOURCE_CONTROL_UNSUPPORTED_LAYOUT` when the worktree and its repository sit on different volumes; bytes that cannot be put back are held as `unrestored` with `effect: unknown`; a batch failure after an earlier target's effect is `effect: unknown`, never `notApplied`), `commit { message, expectedSnapshotId }` (refused with `SOURCE_CONTROL_STATE_CHANGED` when the reviewed `snapshotId` is empty or no longer matches — branch, HEAD, index tree, or merge state — or `GIT_UNMERGED_PATHS` while conflicts remain; once Git publishes the commit it stands, and paths a hook rewrote or added beyond the reviewed tree are reported as `hookChangedPaths` and `hookAddedPaths`), `push`, `pull`. Mutations are serialized per repo (lock keyed by the physical git dir), return the fresh status, and are never cancelled mid-command; every mutation error carries `details.effect: "notApplied" | "unknown"`, never inferred from the error code's `GIT_*` prefix. Timeouts are strictly nested per Git command < host→WSL request < UI request and cover the summed worst case of one request: agent per command 20/60/120/180 s (read/index/commit/remote); host→WSL 120/280/340/380/420 s (status-diff/stage-unstage/discard/commit/push-pull); UI 150/310/370/410/450 s. Discard is its own timeout class at every tier.
- `shutdown {} → shutdownResult { drained: boolean, activeMutations: number }`; the agent stops admitting new mutations (new ones return `AGENT_DRAINING`; reads still served) and waits for any in-flight mutation to finish. `drained: false` never triggers an exit: the agent keeps waiting up to a 450 s emergency bound (longer than any bounded mutation), then terminates its owned process tree, logs `unknown` with the residue, and exits; the response is sent at `drained: true` or at the emergency bound. The host agent forwards `shutdown` to the WSL backend first, treating `WSL_BACKEND_UNAVAILABLE` as drained only when no forwarded mutation request id is still outstanding, then drains itself; SIGTERM/ctrl-c take the same path. The Tauri supervisor sends `shutdown` on stop/restart/exit and labels the stop drained only on an explicit `drained: true` ack; on `drained: false` or no ack it waits for process exit up to 480 s before its existing kill path runs as the emergency bound, logging `unknown` (never `drained`) if the process disappears without an ack, and skips WSL distro termination while finality is unknown. On Windows, the Git runner's children live in a Job Object with no kill-on-close limit, and the emergency stop terminates that job explicitly, taking the whole descendant tree with it; the supervisor likewise spawns the host agent into a job of its own and terminates it on the emergency kill path, so a hung agent's Git descendants are reclaimed with it (an adopted agent has no job and is stopped by binary identity). Helpers that close their pipes outlive Git as on Unix.
