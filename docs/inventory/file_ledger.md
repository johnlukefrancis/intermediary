...
app/src/hooks/stream/stream_tile_records.ts - The panel's tile pixel records: statuses, Blob release, the decoded-pixel lookup, and the published projection
...
app/src/hooks/stream/use_stream_images.ts - Per-panel image tiles keyed by strip and path: revision-bound readImageFile reads, decoded Blob tiles, bounded retent...
...
app/src/lib/stream/stream_tile_pixels_test.ts - Revision-bound pixels: a read is accepted only for the announced bytes and mtime; the decoded-size gate; the mid-read...
app/src/lib/stream/stream_tile_pixels.ts - Pure pixel-acceptance rules for a strip tile: the revision a read must match, the mid-read rewrite refusal, and the d...
app/src/lib/stream/stream_tile_targets_test.ts - Tile retention arithmetic: repo-scoped keys, newest MAX_IMAGE_TILES across strips, the byte and pixel budgets, unfetc...
