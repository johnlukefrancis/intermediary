...
| UI hooks | `app/src/hooks/stream/{use_stream_host, use_repo_stream, use_stream_follow, use_stream_images}.ts` | One agent subscription for all repos (`StreamHost`, mounted once in `app.tsx`); `useSyncExternalStore` binding for the active repo; follow-scroll with the unread pill; bounded image tiles keyed by strip and path (`IMAGE_CARD_MAX_BYTES` 25 MiB = the agent's own `MAX_IMAGE_FILE_BYTES`, sent as the request's `maxBytes`, so the strip previews exactly what the viewer opens; `IMAGE_FETCH_CONCURRENCY` 2; `MAX_IMAGE_TILES` 24; each retained tile is a thumbnail within `STRIP_THUMB_MAX_PX` 1024 produced by `stream_tile_thumbnail.ts` — probe, `MAX_TILE_PIXELS` 64 MP gate before any decode, `createImageBitmap` downscale to WebP, source kept as-is when it already fits — so memory follows the tile count, never the source size; Blob URLs revoked on release). Fetched bytes are accepted only when their length and `mtimeMs` match the payload that produced the card; a mismatch renders `IMAGE CHANGED` and decodes nothing. |
