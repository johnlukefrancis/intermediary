...
| Agent shutdown owners | `crates/im_agent/src/server/shutdown.rs` (`wait_for_shutdown_signal`) + `server/stdin_eof.rs` | Three owners, one drain: SIGTERM, the authenticated `shutdown` command, and EOF on the supervisor's stdin pipe. The pipe is claimed only when fd 0 is a pipe or socket (`fstat`, `S_IFIFO`/`S_IFSOCK`), so a tty or `/dev/null` launch is untouched and a terminal or script run behaves exactly as before; the reader is one `spawn_blocking` task that ends at EOF or with the process. A read failure retires the owner and parks rather than shutting down for a reason nobody asked for. Recorded as `reason: "stdin-eof"` everywhere the drain is logged. |
| WSL emergency stop (process-tree owner) | `src-tauri/src/lib/agent/wsl_agent_termination.rs` + `wsl_agent_termination_channel.rs` + `wsl_process_tree_commands.rs`; budget in `supervisor/wsl_runtime.rs` | TERM the matching agent pids, then wait out the agent's own drain for `WSL_TERMINATE_BUDGET.drain_wait` — the same `HOST_STOP_WAIT_BOUND` (480 s) constant the host stop uses, one owner for both — polling `ss`/`pgrep` at 50 ms for the first second and 1 s thereafter, because each poll is a `wsl.exe` round trip. Exit inside the envelope is `outcome=drained_by_agent`. On expiry, one in-distro script enumerates each agent's descendants from `ps -e -o pid=,ppid=,pgid=`, `kill -KILL`s every descendant process group whose pgid differs from the agent's, then the same-group descendants pid by pid, then the agent — the agent's own group is never group-killed because `wsl.exe` put unrelated processes in it. The count it reports travels into the log as `signalledTrees`. An already-exited agent returns `NoMatch` on the first probe, so the ordinary stop/restart is unchanged. |
| WSL agent stdin pipe | `src-tauri/src/lib/agent/wsl_process_control.rs` (`spawn_wsl_agent_process`) + `supervisor/state.rs` | The backend is spawned with `Stdio::piped()` stdin the supervisor never writes to and never `take()`s: the write end lives inside the `Child` in `SupervisedChild`, so it closes exactly when the supervisor stops intending the process to run — including when the supervisor itself dies. An adopted backend has no `Child` and therefore no pipe, logged `stdin_pipe=none` at the adoption; a spawned one logs `stdin_pipe=held`. |
...
6. Shutdown routes UI/supervisor → host agent → WSL agent: the Tauri supervisor (stop/restart/exit, or
   ...
   and skips WSL distro termination while finality is unknown. Behind that, the supervisor's WSL emergency
   route waits the same 480 s envelope for the WSL agent's own drain before it terminates the agent's
   descendant process groups and then the agent; EOF on the supervisor's stdin pipe asks the WSL agent for
   the same drain when the supervisor dies without sending anything.
...
- The WSL agent has an owner too, and it is not a job object: inside the distro the supervisor cannot
  hold a handle, so the owner is the emergency route's in-distro sweep of the agent's descendant process
  groups. It runs only after the agent's own 450 s drain envelope has expired — before that the agent is
  the owner and killing into it is what leaves `.git/index.lock` behind. A descendant that called
  `setsid` has left the tree and is not reached (accepted boundary).
...
| WSL agent still draining when the supervisor's emergency stop runs | TERM, then the full 480 s drain envelope; an exit inside it logs `outcome=drained_by_agent` and nothing is killed. Only on expiry are the agent's descendant process groups terminated, then the agent, with `signalledTrees=<n>` in the log. |
| The Tauri process dies without sending `shutdown` or SIGTERM | The WSL agent's stdin pipe closes with it; the agent takes the same drain as SIGTERM (`reason: "stdin-eof"`) instead of running on unowned. An adopted WSL agent has no pipe and keeps running until the next launch reclaims it by port. |
