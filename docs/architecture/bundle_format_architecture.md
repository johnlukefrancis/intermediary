...
Bundle format version 2 reserves four generated root entries:

| Entry | Contract |
| --- | --- |
| `BUNDLE_MANIFEST.json` | Machine-readable entry point for build, selection, accounting, and versioned Git-capture facts. |
| `BUNDLE_GIT_STATUS.txt` | Human/model-readable HEAD orientation, selected staging/worktree state, diff stat, name-status, omitted-change count, and capture issues. |
| `BUNDLE_GIT_DIFF.patch` | Selected tracked final-state delta from captured HEAD. Empty for a clean selected tracked state or unavailable evidence. |
...
`fileCount` includes ordinary selected files and all four generated entries. `totalBytesBestEffort` is the converged sum of their uncompressed byte lengths, including the manifest itself.
