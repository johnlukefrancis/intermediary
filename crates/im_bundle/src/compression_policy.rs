...
mod tests {
    ...
    fn stores_for_known_compressed_extensions() {
        assert_eq!(compression_method_for("image.PNG", 10), CompressionMethod::Stored);
        assert_eq!(compression_method_for("video.mp4", 10), CompressionMethod::Stored);
        assert_eq!(compression_method_for("fonts/font.woff2", 10), CompressionMethod::Stored);
        assert_eq!(compression_method_for("archive.tar.gz", 10), CompressionMethod::Stored);
    }

    #[test]
    fn deflates_for_code_by_default() {
        assert_eq!(compression_method_for("src/main.rs", 1024), CompressionMethod::Deflated);
        assert_eq!(compression_method_for("docs/readme.md", 1024), CompressionMethod::Deflated);
    }

    #[test]
    fn stores_for_large_files_even_if_text() {
        let size = STORE_THRESHOLD_BYTES + 1;
        assert_eq!(compression_method_for("logs/big.txt", size), CompressionMethod::Stored);
    }
}
