// Path: crates/im_agent/src/source_control/status/stamp.rs
...
/// neither stamped nor called missing, and is never claimed to be gone. The
/// wire then carries no assertion about that path at all, and a discard aimed
/// at it is refused rather than guessed at: nothing here ever identified the
/// file the user would be destroying.
pub(in crate::source_control) struct DiskState {
    pub stamp: Option<SourceControlWorktreeStamp>,
    pub missing: bool,
}

pub(in crate::source_control) fn stamp_of(path: &Path) -> DiskState {
