// Path: crates/im_agent/src/source_control/discard/tests_stamps.rs
...
/// A rename's origin endpoint is the one target the UI sends with no
/// assertion at all: no stamp, and no claim that it is missing either. It is
/// only ever restored, and while the path really is gone that restore must
/// keep working.
#[tokio::test]
async fn a_target_with_no_assertion_at_all_still_restores_an_absent_path() {
    let (_temp, root) = init_repo_with_commit();
    std::fs::remove_file(root.join("base.txt")).expect("delete tracked file");
    ...
    act(
        &root,
        Action::Discard {
            targets: vec![target("base.txt", None)],
        },
    )
    .await;
...
/// The same target shape pointed at something that is actually there is a
/// different matter: nothing stamped it, so nothing can prove a discard would
/// touch what the user reviewed. That is refused by name rather than quietly
/// restored over, and nothing on disk moves.
#[tokio::test]
async fn a_target_with_no_assertion_over_a_path_that_exists_is_refused_by_name() {
    let (_temp, root) = init_repo_with_commit();
    write(&root, "dir/inside.txt", b"not the reviewed file\n");

    let error = try_act(
        &root,
        Action::Discard {
            targets: vec![target("dir", None)],
        },
    )
    .await
    .expect_err("a directory is not a file any review could stamp");

    assert_eq!(error.code(), "SOURCE_CONTROL_STATE_CHANGED");
    assert_eq!(
        error.message(),
        "cannot identify dir before discarding it (not a regular file the review could stamp)"
    );
    assert_eq!(error.effect(), Some("notApplied"));
    assert_eq!(read(&root, "dir/inside.txt"), b"not the reviewed file\n");
}
