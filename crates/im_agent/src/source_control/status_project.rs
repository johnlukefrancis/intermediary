...
impl Lists {
    ...
    /// A record whose current path is outside the root. Only staged content
    /// travels with a commit of the whole index; worktree-only, untracked, and
    /// unmerged records out there are not this root's concern. A staged rename
