...
use super::discard_quarantine::sweep_stale_quarantine;
use super::locks::SourceControlLocks;
use super::runner::{self, GitCall, READ_TIMEOUT, STATUS_LIMIT};
use super::status_index_tree::capture_index_tree_sha;
use super::status_project::project_status;
use super::status_stamp::stamp_worktree_entries;
...
/// against, then Git decides `committable` and the index identity is
/// computed read-only.
///
...
pub(super) async fn capture_status(
    ...
    if locks.mark_quarantine_swept(&location.git_dir) {
        sweep_stale_quarantine(&location.git_dir).await;
    }
